...
#### Rule
> multiply by 10ⁿ to store · convert to/from binary · divide by 10ⁿ to read back
...
When you are selecting which Q format to use you exchange range for precision
...
Encoding Steps (Decimal → int128)
- Multiply decimal by $2^{64}$ (or left-shift integer part `<<` 64, add fractional scaled).
- Round to the nearest int128 (depends on library).
- Store as a signed 128-bit value.

#### Decoding

Decoding Steps (int128 → Decimal)
- Divide stored int128 by $2^{64}$
- The result is the exact decimal (or approximation if non-exact).

#### Rule
> multiply by 2ⁿ, or shift `<< n` to store · divide by 2ⁿ or `>> n` to read back

...
Arbitrage is a double-edge sword:
...
A Uniswap V2 pool, when is arbitraged, its tokens are balanced 50/50 by token value, not by token counts.
...
> 50:50 token value balance is not as a hard guarantee from the contract itself, it's the result of arbitrage
...
When the AMM rebalances, it exchanges some of the outperforming tokens with underperming tokens in `local price`, `not market price`.
...
Trading (including arbitrage) in UnisWay does not reduce the total token reserves; it only changes their composition.
